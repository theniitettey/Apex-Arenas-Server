...
### 9. `auth.login.controller.ts`

- [ ] **Use `response.utils.ts` helpers** → Replace manual `res.json()`:
  - `sendSuccess()` for successful responses
  - `sendError()` for error responses
  - `sendUnauthorized()` for 401 responses
  - `sendForbidden()` for 403 responses
- [ ] **Use `AUTH_ERROR_CODES`** → Instead of hardcoded string error codes
- [ ] **Use `extractDeviceContext`** → Replace manual ip/user_agent extraction
- [ ] **Use `getAuditMetadata`** → For audit log metadata

---

### 10. `auth.otp.controller.ts`

- [ ] **Use `response.utils.ts` helpers** → Replace manual `res.json()`:
  - `sendSuccess()`
  - `sendError()`
  - `sendValidationError()`
  - `sendRateLimited()` for 429 responses
- [ ] **Use `AUTH_ERROR_CODES`** → Replace hardcoded string error codes
- [ ] **Use `extractDeviceContext`** → Replace:
  ```typescript
...
### 11. `auth.admin.controller.ts`

- [ ] **Use `response.utils.ts` helpers** → Replace manual `res.json()`
- [ ] **Use `AUTH_ERROR_CODES`** → Replace hardcoded string error codes
- [ ] **Use `extractDeviceContext`** → Replace manual extraction pattern:
  ```typescript
  ip_address: req.ip || 'unknown',
  user_agent: req.get('user-agent') || 'unknown'
  ```
- [ ] **Use `getAuditMetadata`** for device_context params

---

### 12. `auth.password.controller.ts`

- [ ] **Use `response.utils.ts` helpers**
- [ ] **Use `AUTH_ERROR_CODES`**
- [ ] **Use `extractDeviceContext`**

---

### 13. `auth.register.controller.ts`

- [ ] **File appears incomplete** - needs full implementation
- [ ] **Use `response.utils.ts` helpers**
- [ ] **Use `AUTH_ERROR_CODES`**
- [ ] **Use `extractDeviceContext`**
...
### 14. `auth.jwt.middleware.ts`

- [ ] **Use `AUTH_ERROR_CODES`** → Replace hardcoded strings:
  - `'MISSING_TOKEN'` → `AUTH_ERROR_CODES.MISSING_TOKEN`
  - `'TOKEN_REVOKED'` → `AUTH_ERROR_CODES.TOKEN_REVOKED`
  - `'INVALID_TOKEN'` → `AUTH_ERROR_CODES.INVALID_TOKEN`
  - `'TOKEN_EXPIRED'` → `AUTH_ERROR_CODES.TOKEN_EXPIRED`
  - `'ACCOUNT_INACTIVE'` → `AUTH_ERROR_CODES.ACCOUNT_INACTIVE`
  - `'ACCOUNT_BANNED'` → `AUTH_ERROR_CODES.ACCOUNT_BANNED`
  - `'PASSWORD_CHANGE_REQUIRED'` → `AUTH_ERROR_CODES.PASSWORD_CHANGE_REQUIRED`
  - `'ADMIN_REQUIRED'` → `AUTH_ERROR_CODES.ADMIN_REQUIRED`
  - `'ADMIN_INACTIVE'` → `AUTH_ERROR_CODES.ADMIN_INACTIVE`
  - `'IP_BLOCKED'` → `AUTH_ERROR_CODES.IP_BLOCKED`
- [ ] **Use `extractDeviceContext`** → For ip/user_agent

---

### 15. `auth.admin.middleware.ts`

- [ ] **Use `AUTH_ERROR_CODES`** → Replace hardcoded strings:
  - `'AUTH_REQUIRED'` → `AUTH_ERROR_CODES.AUTH_REQUIRED`
  - `'ADMIN_NOT_WHITELISTED'` → `AUTH_ERROR_CODES.ADMIN_NOT_WHITELISTED`
  - `'SUPER_ADMIN_REQUIRED'` → `AUTH_ERROR_CODES.SUPER_ADMIN_REQUIRED`
  - `'SELF_ACTION_NOT_ALLOWED'` → `AUTH_ERROR_CODES.SELF_ACTION_NOT_ALLOWED`

---

### 16. `auth.ratelimit.middleware.ts`

- [ ] **Use `AUTH_ERROR_CODES.RATE_LIMITED`** → Instead of hardcoded `'RATE_LIMIT_EXCEEDED'`
