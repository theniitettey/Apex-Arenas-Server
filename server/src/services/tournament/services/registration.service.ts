...
export class RegistrationService {
  ...
  private async _registerInternal(
    ...
    try {
      logger.info("Starting registration flow (LOCKED)", {
        tournamentId,
        userId,
      });

      // 1. Validate registration eligibility (using validation service)
      await tournamentValidationService.validateCanRegister(
        tournamentId,
        userId,
      );
      ...
      let status: "pending_payment" | "registered" | "waitlist" =
        "pending_payment";
      ...
      if (
        ...
        try {
          ...
          registration.payment = {
            ...
            transaction_id:
              paymentResult.transactionId || new mongoose.Types.ObjectId(),
            paid_at: new Date(),
          };
          ...
          logger.info("Payment successful, registration confirmed", {
  ...
  private async _unregisterInternal(
    ...
    try {
      ...
      const isBeforeCutoff =
        tournament.schedule.cancellation_cutoff &&
        now < tournament.schedule.cancellation_cutoff;
  ...
  async promoteFromWaitlist(
    tournamentId: string,
  ): Promise<IApexRegistration | null> {
    return redisLock.executeWithLock(
  ...
  private async _promoteFromWaitlistInternal(
    tournamentId: string,
  ): Promise<IApexRegistration | null> {
    try {
      ...
      // Promote user
      waitlistedReg.status = tournament.is_free
        ? "registered"
        : "pending_payment";
      ...
      // Update counts
      tournament.capacity.waitlist_count = Math.max(
        0,
        tournament.capacity.waitlist_count - 1,
      );
      if (tournament.is_free) {
      ...
      if (error instanceof AppError) throw error;
      logger.error("Waitlist promotion failed", {
        tournamentId,
        error: error.message,
      });
      return null;
  ...
  private async processPayment(
    ...
    logger.info("Processing payment (placeholder)", {
      registrationId,
      paymentData,
    });
  ...
  async listByTournament(
    ...
    try {
      ...
      if (filters.checked_in !== undefined)
        query["check_in.checked_in"] = filters.checked_in;

      const registrations = await Registration.find(query)
        .populate(
          "user_id",
          "username profile.first_name profile.last_name profile.avatar_url",
        )
      ...
      logger.error("List registrations failed", {
        tournamentId,
        error: error.message,
      });
      throw new AppError(
  ...
  async listByUser(
    ...
    try {
      ...
      logger.error("List user registrations failed", {
        userId,
        error: error.message,
      });
      throw new AppError(
  ...
  async getById(registrationId: string): Promise<IApexRegistration> {
    try {
      ...
      if (error instanceof AppError) throw error;
      logger.error("Get registration failed", {
        registrationId,
        error: error.message,
      });
      throw new AppError(
